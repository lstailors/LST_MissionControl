...
// ═══════════════════════════════════════════════════════════
// Message Bubble — premium chat message rendering
// ═══════════════════════════════════════════════════════════
...
export const MessageBubble = memo(function MessageBubble({ message, onResend }: MessageBubbleProps) {
  ...
  // Clean content — ensure it's always a plain string
  ...
  // Clean user messages — remove envelope metadata
  ...
  return (
    ...
    >
      {/* Avatar */}
      {isUser ? (
        <div className="w-8 h-8 rounded-xl bg-aegis-elevated border border-aegis-border/40 flex items-center justify-center shrink-0 mt-0.5 shadow-inner-glow">
          <User size={14} className="text-aegis-text-muted" />
        </div>
      ) : (
      ...
      <div className={clsx('flex flex-col max-w-[80%] min-w-0', isUser && 'items-end')}>
        ...
        <div
          className={clsx(
            ...
            isUser
              ? 'bg-aegis-user-bubble border border-aegis-user-border rounded-tl-md'
              : 'bg-aegis-bot-bubble border border-aegis-bot-border rounded-tr-md shadow-inner-glow',
        ...
        >
              ...
              {message.attachments
                ...
                .map((att, i) => (
                  <img
                    ...
                    className="max-w-[280px] max-h-[200px] rounded-xl border border-aegis-border/30 cursor-pointer hover:opacity-90 hover:shadow-glow-sm transition-all"
                    loading="lazy"
          ...
          {isUser ? (
            // User message — plain text, nice typography
                          ...
                          return (
                            <div className="my-2">
                              <img
                                src={src}
                                alt={alt || ''}
                                className="max-w-full max-h-[400px] rounded-xl border border-aegis-border/30 cursor-pointer hover:opacity-90 hover:shadow-glow-sm transition-all"
                                loading="lazy"
                              ...
                              />
                              {alt && <span className="text-[11px] text-aegis-text-dim mt-1 block">{alt}</span>}
                            </div>
                          );
                        },
                        a({ href, children }: any) {
                          return (
                            <a
                              ...
                              className="text-aegis-primary hover:text-aegis-primary-hover"
                            >
                              {children}
                            </a>
                          );
        ...
        <div className="flex items-center gap-2 mt-1 px-1 h-5">
          <span className="text-[10px] text-aegis-text-dim/60 font-mono">{timeStr}</span>
            ...
            <div className="flex items-center gap-0.5 animate-fade-in">
              ...
              >
                {copied ? (
                  ...
                  <Copy size={11} className="text-aegis-text-dim/60 hover:text-aegis-text-muted" />
                )}
              </button>
              {isUser && onResend && (
                <button
                  ...
                  title={t('chat.resend')}
                >
                  <RotateCcw size={11} className="text-aegis-text-dim/60 hover:text-aegis-text-muted" />
                </button>
              )}
            </div>
          )}
